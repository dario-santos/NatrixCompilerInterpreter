...
let size_of_value = function
  | Vint n     -> (n - minint) 
  | Vset(i,f)  -> (f - i) + 1
  | Vlist(_,r) -> let i,f = vset_to_tuplo r in (f - i) + 1

(* Funcao print *)
let rec print_value = function
  | Vint n -> if n < minint || n > maxint then error "The print function does not support negative numbers" else printf "%d" n
...
let binop op v1 v2 = match op, v1, v2 with
  ...
  | Bdiv, Vint n, Vint 0 -> error ("Illegal expression: Division by zero, you are trying to do following operation: " ^ string_of_int n ^ " / 0.")  
  | Bmod, Vint n, Vint 0 -> error ("Illegal expression: Division by zero, you are trying to do following operation: " ^ string_of_int n ^ " % 0.")  
  | Bmod, Vint n1, Vint n2 -> Vint (n1 mod n2)
  | Bdiv, Vint n1, Vint n2 -> Vint (n1 / n2)
  | Beq,  Vint n1, Vint n2 -> if compare v1 v2 = 0  then Vint 1 else Vint 0
  | Bneq, Vint n1, Vint n2 -> if compare v1 v2 <> 0 then Vint 1 else Vint 0
  | Blt,  Vint n1, Vint n2 -> if compare v1 v2 < 0  then Vint 1 else Vint 0
  | Ble,  Vint n1, Vint n2 -> if compare v1 v2 <= 0 then Vint 1 else Vint 0
  | Bgt,  Vint n1, Vint n2 -> if compare v1 v2 > 0  then Vint 1 else Vint 0
  | Bge,  Vint n1, Vint n2 -> if compare v1 v2 >= 0 then Vint 1 else Vint 0
  | _ -> assert false
...
let rec expr ctxs = function
  ...
  | Eset (e1, e2) -> 
      ...
      if i >= 0 && f >= 0 && i <= f && f - i > 0 then Vset (i, f) 
      else error ("Invalid size of set. A set needs to have atleast the size of one, you are trying to do: [" ^ string_of_int i ^ " .. " ^ string_of_int f^ "], try : [0 .. 1].")
  | Eminint -> Vint minint
  | Emaxint -> Vint maxint
  | Ebinop (Band, e1, e2) ->
    ...
    begin try
      ...
      with 
        | Return r ->
            ...
            else error ("Value out of bounds. The value " ^ string_of_int r  ^ " can\'t be used as the return of  " ^ f ^ ", try a value in the range " ^ string_of_value return ^ ".");
    end
  | Eident id ->
  ...
  | Eget (id, e1) ->
      ...
      if index - i < 0 || index > f then error ("Value out of bounds. The value " ^ string_of_int index  ^ " can\'t be used with the array " ^ id ^ ", try a value in the range " ^ string_of_value range ^ ".");
...
and interpret_stmt ctxs = function
  ...
  | Sassign (id, e1)  ->
      ...
      else error ("Value out of bounds. The value " ^ string_of_int v1 ^ " can\'t be used with the variable " ^ id ^ ", try a value in the range " ^ string_of_value t1 ^ ".")
  ...
  | Sdeclare (id, t ,e1) ->
      ...
      else error ("Value out of bounds. The value " ^ string_of_int v1 ^ " can\'t be used with the variable " ^ id ^ ", try a value in the range " ^ string_of_value tp ^ ".")
  ...
  | Sdeclarearray (id, ida, e) ->
      ...
      if not (value_in_type_limits v1 t1) then error ("Value out of bounds. Error declaring an array, the value " ^ string_of_int v1  ^ " can\'t be used with the variable " ^ id ^ ", try a value in the range " ^ string_of_value t1 ^ ".");
      
      (* 4 - Se tudo correu bem entao vamos adicionar *)
      let sz = size_of_value range in
      let arr = Array.make sz (Vint v1) in
  ...
  | Sarray (id, sz, t) -> 
      ...
      if sv <= 0 then error ("Error defining " ^ id ^ " array. An array must have more than 0 elements but was givin " ^ string_of_int sv ^ "."); 
      
      (* 3 - Verificar que tp esta contido nos limites do tipo*)
      let t1 = expr ctxs t in
      if size_of_value t1 <= 0 then error ("Error defining the array " ^ id ^  ". The type of an arrray can't have a range of 0 or be negative."); 
  ...
  | Saset (id, e1, e2) ->
      ...
      if not (value_in_type_limits v1 t1) then error ("Value out of bounds. Error assigning the value " ^ string_of_int v1  ^ " can\'t be used with the array " ^ id ^ ", try a value in the range " ^ string_of_value t1 ^ ".");
