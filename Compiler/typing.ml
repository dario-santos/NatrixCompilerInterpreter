...
let rec stmt ctxs = function
  | Sif (e, s1, s2) ->
      ...
      stmt (ctxs@[(Hashtbl.create 17 : table_ctx)]) s1;
...
and stmts ctxs = function  
  | Stfunction (f, args, return, body) -> 
    (*
      1 - f ainda usado                              -
      2 - Todos argumentos são inteiros ou conjuntos -
      3 - verificar o corpo                          -
      4 - Adicionar à tabela de funções              -
    *)
    if List.length(find_id f ctxs) != 0 then error ("Lexical analysis: The identifier " ^ f ^ " is already in use.");
