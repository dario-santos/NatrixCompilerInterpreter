...
let rec verify_expr ctxs = function
  ...
  | Ecall (f, el) ->
      ...
      (* 5 - Se tudo correu bem entao retornamos o tipo de retorno da funcao *)
      costumtype_to_typ ctxs return
...
let rec verify_stmt ctxs = function
  ...
  | Sreturn e ->
    ...
    let t1 = verify_expr ctxs e in
    if not(is_int t1) then error ("Lexical anali«ysis: The return statement only supports integers.");
  ...
  | Sdeclare (id, _, e) ->
      ...
      (* 2 - Verificar se e do tipo Tint*)
      let t1 = verify_expr ctxs e in
      if not( is_int t1) then error ("Lexical analysis: the declaration statement only supports integers but was given " ^ string_of_typ t1 ^ ".");
  ...
  | Sset (id, set) ->
      (* 1 - Verificar se o nome ja nao esta a ser usado *)
      let ctx = List.hd (List.rev ctxs) in
      if Hashtbl.mem functions id then error ("Lexical analysis: Error defining a set. The identifier " ^ id ^ " is already defined.");     
      if Hashtbl.mem ctx id then error ("Lexical analysis: Error defining a set. The identifier " ^ id ^ " is already defined.");
  
      (* 2 - Verificar se estamos a lhe dar um set *)
      let t1 = verify_expr ctxs set in
      if not(is_set t1) then error ("Lexical analysis: Error defining a set. Was expecting a Tset but was given " ^ string_of_typ t1 ^ ".");
      Hashtbl.add ctx id t1

  | Sprint e ->
      (* 1 - Verificar se estamos a receber um Tint*)
      let t1 = verify_expr ctxs e in
      if not (is_int t1) then error ("Lexical analysis: The print statement only supports Tint but was given a " ^ string_of_typ t1^".")
...
and verify_stmts ctxs = function  
  | Stfunction (f, args, return, body) -> 
    ...
    let function_ctx = (Hashtbl.create 17 : table_ctx) in
    let rec verify_arguments = function
      | hd::tl ->
        (* 2.1 - Verificar se o nome e unico *)
        let name, t = hd in
        let tp = costumtype_to_typ ctxs t in
        if Hashtbl.mem function_ctx name then error ("Lexical analysis: There are multiple arguments with the same identifier in the function " ^ f ^ "."); 
        ...
        Hashtbl.add function_ctx name Tint;
        verify_arguments tl
    ...
    verify_stmt (ctxs@[function_ctx]) body;
    
    (* 5 - Se estiver tudo em ordem entao guarda a funcao *)
    Hashtbl.add functions f (function_ctx, return, body)
